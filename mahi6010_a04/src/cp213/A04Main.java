...
public class A04Main {
	...
	final ArrayList<CountedStore<Character>> array = tree.inOrder();
	int totalCount = 0;

	for (final CountedStore<Character> cs : array) {
	...
	for (final CountedStore<Character> cs : array) {
	    ...
	    final double percent = (double) cs.getCount() / totalCount * 100;
	    System.out.format("%4s %,8d %7.2f%n", cs.getDatum(), count, percent);
	}
	return;
    }
	...
	for (final char letter : string.toCharArray()) {
	    final CountedStore<Character> data = new CountedStore<Character>(Character.toUpperCase(letter));
	    tree.insert(data);
	}
	return;
    }
	...
	for (Integer i : testData) {
	    CountedStore<Integer> cs = new CountedStore<>(i);
	    source.insert(cs);
	}
	...
	CountedStore<Integer> key = new CountedStore<>(3);
