...
/**
 * The individual node of a linked structure that stores CountedStore objects.
...
public class TreeNode<T extends Comparable<T>> {
    ...
    // Attributes
    private CountedStore<T> cs = null; // the node data
    ...
    /**
     * Creates a new TreeNode with data cs and null links to its child TreeNodes.
     *
     * @param cs The data to store in the node.
     */
    public TreeNode(final CountedStore<T> cs) {
	this.cs = cs;
    }
     ...
     */
    private final void inOrderAux(final TreeNode<T> node, final ArrayList<CountedStore<T>> list) {
	if (node != null) {
	    this.inOrderAux(node.getLeft(), list);
	    list.add(new CountedStore<T>(node.getCs()));
     ...
     * @param list The final list of node data.
     */
    private final void preOrderAux(final TreeNode<T> node, final ArrayList<CountedStore<T>> list) {
	if (node != null) {
	    list.add(new CountedStore<T>(node.getCs()));
     ...
     */
    public final ArrayList<CountedStore<T>> inOrder() {
	final ArrayList<CountedStore<T>> list = new ArrayList<>();
     ...
     */
    public final ArrayList<CountedStore<T>> levelOrder() {
	final ArrayList<CountedStore<T>> list = new ArrayList<>();
	...
	if (this != null) {
		...
		node = queue.remove(0);
		// Add a copy of the node cs to the list of cs
		list.add(new CountedStore<T>(node.getCs()));
     ...
     * @return The contents of this tree as a list of data.
     */
    public final ArrayList<CountedStore<T>> preOrder() {
	final ArrayList<CountedStore<T>> list = new ArrayList<>();
	...
	return "D: " + this.cs + "; H: " + this.height;
    }
